...
MATCHES_TEMPLATE_ID = 3318704   # ← Reemplaza con el ID real de "Lista de Partidos 2025"
EVENTS_TEMPLATE_ID = 3315314    # ← Reemplaza con el ID real de "Eventos en Partidos 2025"
        ...
        while True:
            ...
            try:
                ...
                if report_name not in ["Eventos en Partidos", "Match Events"]:
                    ...
                    continue  # ← usa continue, no break, para probar con otras API_KEY

                # Extraer temporada de forma segura
                results = data.get("results", [])
                season = results[0].get("season") if results else None
                if not season or season != "2025":
                    print(f"⏭️  Temporada ignorada: {season}")
                    continue  # ← no detener, seguir con otras API_KEY
